...
subcor_mask=logical(tmp>0).*logical(tmp<72); % generate the subcortical mask
...
V=normalize(V,2,'norm'); % calculate the normal direction to the boundary
...
ALFF_G=interp3(linspace(1,size(data,2),size(data,2)),linspace(1,size(data,1),size(data,1)),linspace(1,size(data,3),size(data,3)),data,g2,g1,g3);% extrct the image value on gray matter vertices using trilinear.
ALFF_W=interp3(linspace(1,size(data,2),size(data,2)),linspace(1,size(data,1),size(data,1)),linspace(1,size(data,3),size(data,3)),data,w2,w1,w3);% extrct the image value on white matter vertices using trilinear.
ALFF_G(isnan(ALFF_G))=0;
ALFF_W(isnan(ALFF_W))=0;
ALFF_W2G=ALFF_W./(ALFF_G+eps);% calculate the GWBP
